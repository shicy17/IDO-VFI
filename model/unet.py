...
import utils.size_adapter as size_adapter
...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # s5 = self.down4(s4)
        x = self.down4(s4)
        # print('s:', s1.shape, s2.shape, s3.shape, s4.shape, s5.shape)
        # x = self.down5(s5)  # s5 512
        x = self.up1(x, s4)
        x = self.up2(x, s3)
        x = self.up3(x, s2)
        x = self.up4(x, s1)
...
# 2-60w
class UNet_60w(nn.Module):
    def __init__(self, inChannels, outChannels, ends_with_relu=True):
        super(UNet_60w, self).__init__()
        self._ends_with_relu = ends_with_relu
        self._size_adapter = size_adapter.SizeAdapter(minimum_size=32)
        self.conv1 = nn.Conv2d(inChannels, 32, 7, stride=1, padding=3)
        self.conv2 = nn.Conv2d(32, 32, 7, stride=1, padding=3)
...
if __name__=='__main__':
    x=th.rand([10,3,122,122])
    unet=UNet(3,20)
    print(unet.forward(x).size())